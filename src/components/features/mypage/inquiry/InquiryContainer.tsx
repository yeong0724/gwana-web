...
const InquiryContainer = () => {
  ...
  return (
    <div className="bg-warm-50 h-[calc(100dvh-56px)] flex flex-col overflow-hidden">
      <div className="mx-auto w-full flex-1 flex flex-col min-h-0 max-w-[600px] border-x border-brand-100 bg-warm-50">
        {/* 헤더: 타이틀 */}
        <div className="flex items-center justify-between py-2.5 border-b border-brand-200 px-4 bg-white sticky top-0 z-10">
        ...
        {/* 날짜 범위 선택 */}
        <div className="flex items-center gap-2 px-4 py-3 border-b border-brand-100 bg-brand-50/50">
          {/* 시작 날짜 */}
          <div className="w-[40%]">
            <DatePicker
              name="startDate"
        ...
        {inquiryList.length === 0 ? (
          // 빈 상태 UI
          <Card className="flex-1 flex flex-col border-0 shadow-none bg-warm-50">
            <CardContent className="p-0 flex-1 flex flex-col">
              <div className="flex-1 flex flex-col items-center justify-center text-warm-400">
                {!isFetching && (
                  <>
                    <MessageCircleQuestion className="size-16 mb-4 text-brand-200" />
          ...
          <div className="flex-1 min-h-0 overflow-y-auto relative bg-white">
                ...
                <li key={index} className="group">
                  ...
                  >
                    <div className="flex-1 min-w-0 text-left">
                      <p className="text-[14px] text-brand-700 font-semibold truncate group-hover:text-tea-600 transition-colors">
                        {title}
                      </p>
                        ...
                        <span
                          className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                            isAnswered === YesOrNoEnum.YES
                              ? 'bg-tea-50 text-tea-700 border border-tea-200'
                    ...
                    <ChevronRight className="size-5 text-warm-300 flex-shrink-0 ml-2 group-hover:text-tea-500 group-hover:translate-x-0.5 transition-all" />
                  </button>
                </li>
              ))}
            </ul>
            ...
            {hasNextPage && (
              <div className="sticky bottom-0 left-0 right-0 flex justify-center pointer-events-none">
                <div className="">
                  <ChevronDown className="text-warm-400/90 animate-bounce" size={30} />
                </div>
              </div>
            )}
          </div>
