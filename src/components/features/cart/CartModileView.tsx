...
const CartModileView = () => {
  ...
  return (
    <div className="flex flex-col w-full mx-auto flex-1 border-x border-brand-100 overflow-hidden bg-warm-50">
      {/* 전체선택 헤더 */}
      <div className="flex items-center justify-between p-3 bg-white border-b border-brand-200 flex-shrink-0">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          />
          <span className="text-[14px] font-medium text-brand-900">
            전체선택 ({size(filter(cart, { checked: true }))} / {size(cart)})
          </span>
        </label>
        <button
          disabled={isNoSelect}
          className="text-[13px] text-warm-400 hover:text-brand-800 hover:bg-brand-50 px-2 py-1 transition-colors disabled:text-warm-200 disabled:hover:bg-transparent disabled:cursor-not-allowed"
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {cart.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full gap-3 bg-warm-50">
            <ShoppingCart size={64} className="text-brand-200" strokeWidth={1.5} />
            <span className="text-warm-400 text-[14px]">장바구니가 비어있습니다</span>
          </div>
        ) : (
          <div className="flex flex-col min-h-full">
              ...
              {map(cart, (item, index) => (
                <React.Fragment key={`${item.cartId}-${index}`}>
                  <div key={`${item.cartId}-${index}`} className="bg-white px-2">
                        ...
                        {map(item.cartItems, (cartItem, cartItemIndex) => (
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between">
                              {/* 수량 조절 */}
                              <div className="flex items-center border border-brand-200 overflow-hidden">
                                <button
                                  className="w-7 h-6 hover:bg-brand-50 border-r border-brand-200 flex items-center justify-center disabled:opacity-50 transition-colors"
                                ...
                                <span className="w-8 h-6 text-[12px] text-brand-900 flex items-center justify-center tabular-nums">
                                  {cartItem.quantity}
                                </span>
                                <button className="w-7 h-6 hover:bg-brand-50 border-l border-brand-200 flex items-center justify-center transition-colors">
                                  <Plus
                                    size={12}
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {/* 삭제 버튼 */}
                                <button
                                  className="p-1 hover:bg-brand-100 rounded active:scale-90 transition-transform"
                                  onClick={() =>
                                    onDeleteCart(
                                      item.cartId,
                                      cartItem.cartItemId,
                                ...
                                >
                                  <X size={14} className="text-warm-300" />
                                </button>
                              </div>
                            </div>
                          </div>
                    ...
                    {/* 하단 가격 산정 영역 */}
                    <div className="text-center text-[11px] text-warm-500 pt-3 pb-5 bg-white">
                  ...
                  {cart.length - 1 !== index && <div className="h-px bg-brand-200 scale-y-50" />}
                </React.Fragment>
              ))}
              <div className="h-px bg-brand-200 scale-y-50" />
            </div>
            ...
            {/* 결제 예상 금액 - 스크롤 영역 내 */}
            <div className="bg-white px-3 pt-4 pb-3 border-t border-brand-200">
              <h3 className="text-[15px] font-bold text-brand-900 mb-3">결제 예상 금액</h3>
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[13px]">
                  <span className="text-warm-500">상품금액</span>
                  <span className="text-brand-900 tabular-nums">
                    {localeFormat(totalProductPrice)}원
                  </span>
                </div>
                <div className="flex justify-between items-center text-[13px]">
                  <span className="text-warm-500">배송비</span>
                  <span className="text-brand-900 tabular-nums">
                    {localeFormat(totalShippingPrice)}원
                  </span>
                </div>
                <div className="h-px bg-brand-200 my-3" />
                <div className="flex justify-between items-center text-[15px] font-bold">
                  <span className="text-brand-900">총 결제 금액</span>
                  <span className="text-tea-600 tabular-nums">
      ...
      {/* 구매하기 버튼 - 하단 고정 */}
      <div className="flex-shrink-0 bg-white p-4 z-10 border-t border-brand-200">
        <button
          disabled={isNoSelect}
          className="w-full bg-tea-500 text-white hover:bg-tea-600 rounded-full py-3 flex items-center justify-center gap-2 disabled:bg-warm-200 disabled:cursor-not-allowed disabled:hover:bg-warm-200 transition-colors cursor-pointer"
